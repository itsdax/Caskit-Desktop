...
public class CaskitTrayApp {
    ...
    public CaskitTrayApp() {
        ...
        try {
            Image image = ImageIO.read(new File(CaskitTrayApp.class.getResource("../images/logo_icon.png").getPath())).getScaledInstance(systemTray.getTrayImageSize(), -1, Image.SCALE_SMOOTH);
    ...
    private static Image loadImage(String resourcePath, int size) {
        try {
            return ImageIO.read(new File(CaskitTrayApp.class.getResource(resourcePath).getPath())).getScaledInstance(size, -1, Image.SCALE_SMOOTH);
        } catch (IOException e) {
            return null;
        }
    }
